-- WolfPack Forum — Database Migration
-- Run this in Supabase SQL Editor to set up forum tables
...
-- Allow anyone to update thread counts (for reply_count, view_count increments)
...
INSERT INTO forum_threads (id, category_id, user_id, title, body, post_type, is_pinned, reply_count, view_count, user_name, user_avatar, created_at, last_reply_at) VALUES
  ('00000000-0000-0000-0000-000000000001', 'den', '00000000-0000-0000-0000-000000000000', 'Welcome to The Den — Introduce yourself!',
   E'Hey wolves! 🐺\n\nThis is the place to say hi. Drop a quick intro:\n\n- What are you building?\n- What tools do you use?\n- What''s your superpower?\n\nNo lurking allowed. Even a one-liner counts. Let''s go!',
   'discussion', true, 3, 42, 'WolfPack', null, now() - interval '3 days', now() - interval '1 hour'),

  ('00000000-0000-0000-0000-000000000002', 'guides', '00000000-0000-0000-0000-000000000000', 'How I built a SaaS MVP in 48 hours with AI',
   E'Let me walk you through my process for going from zero to deployed SaaS in a weekend.\n\n## The Stack\n- Next.js 16 + Supabase\n- Claude for code generation\n- Vercel for deployment\n\n## Step 1: Define the core loop\nBefore writing any code, I spent 2 hours defining the ONE thing the product does. Not two things. One.\n\n## Step 2: Schema first\nI designed the database schema before touching the UI. This saved me hours of refactoring.\n\n## Step 3: AI-assisted implementation\nUsed Claude to scaffold the API routes and basic components. Spent my time on the business logic and UX polish.\n\n## Step 4: Ship it\nDeployed to Vercel, shared on Twitter, got 50 signups day one.\n\nThe secret? Don''t overthink it. Ship ugly, iterate fast.',
   'tutorial', true, 5, 128, 'BuilderWolf', null, now() - interval '2 days', now() - interval '3 hours'),

  ('00000000-0000-0000-0000-000000000003', 'showcase', '00000000-0000-0000-0000-000000000000', '🚀 Just launched: AI Invoice Generator',
   E'After 3 weeks of building, I finally shipped my AI invoice generator!\n\n**What it does:** Generate professional invoices from a text description. Just describe the work you did and it creates a formatted PDF.\n\n**Tech stack:** Next.js, OpenAI API, PDF generation with jsPDF\n\n**What I learned:**\n- PDF generation is surprisingly painful\n- AI is great at extracting structured data from natural language\n- Users care about design more than features\n\nWould love feedback from the pack! What features should I add next?',
   'showcase', false, 2, 67, 'ShipItWolf', null, now() - interval '1 day', now() - interval '5 hours'),

  ('00000000-0000-0000-0000-000000000004', 'help', '00000000-0000-0000-0000-000000000000', 'How do you handle auth in Next.js 16?',
   E'I''m building my first Next.js app and struggling with authentication.\n\nQuestions:\n1. Should I use Supabase Auth or NextAuth?\n2. How do you protect API routes?\n3. What''s the best way to handle sessions with the App Router?\n\nAny guidance appreciated! 🙏',
   'question', false, 4, 89, 'NewPup', null, now() - interval '12 hours', now() - interval '2 hours')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO forum_replies (id, thread_id, user_id, body, likes_count, user_name, user_avatar, created_at) VALUES
  ('00000000-0000-0000-0000-100000000001', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000000',
   E'Hey everyone! I''m a full-stack dev building AI tools for small businesses. My superpower: turning coffee into code at 2am. 🐺',
   3, 'NightWolf', null, now() - interval '2 days'),

  ('00000000-0000-0000-0000-100000000002', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000000',
   E'Just joined! I''m a designer who''s learning to code with AI assistance. Building a personal brand toolkit. Excited to be here!',
   2, 'DesignWolf', null, now() - interval '1 day'),

  ('00000000-0000-0000-0000-100000000003', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000000',
   E'Solo founder here. Building in public and loving the accountability. Let''s ship! 🚀',
   1, 'SoloAlpha', null, now() - interval '1 hour'),

  ('00000000-0000-0000-0000-100000000004', '00000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000000',
   E'Supabase Auth is the way to go with Next.js 16. It handles sessions via cookies with the @supabase/ssr package.\n\nFor protecting API routes, check the session in your route handler:\n\n```ts\nconst supabase = createServerSupabaseClient();\nconst { data: { user } } = await supabase.auth.getUser();\nif (!user) return Response.json({ error: "Unauthorized" }, { status: 401 });\n```\n\nWorks great with the App Router!',
   5, 'AuthWolf', null, now() - interval '10 hours'),

  ('00000000-0000-0000-0000-100000000005', '00000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000000',
   E'+1 for Supabase. The magic link flow is super smooth and your users don''t need to remember passwords.',
   2, 'BuilderWolf', null, now() - interval '6 hours')
ON CONFLICT (id) DO NOTHING;
